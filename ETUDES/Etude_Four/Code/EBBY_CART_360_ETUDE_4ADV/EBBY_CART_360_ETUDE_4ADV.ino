...
void setRGB()
{
  switch (mode)
  {
    ...
    case 4:
      {
        analogWrite(LED_PIN_R, 110);   // Turn on the LED -R
        analogWrite(LED_PIN_G, 0);   // Turn on the LED -G
        analogWrite(LED_PIN_B, 177);   // Turn on the LED -B
        break;
      }
    default:
      {
...
void live()
{
  ...
  /* ----- EBBY's stuff ----- */
  testNote = pitchIt(analogRead(NOTE_IN_PIN));
    ...
    */
    if (testNote > theAdjuster && (timePassed < MAX_PRESS_TIME)) {
      ...
      */
      ...
      { // update the timer
        /*** EBBY HAS IMPLEMENTED THE FUNCTION ***/
        updateTimer();
        /*** EBBY HAS IMPLEMENTED THE FUNCTION ***/
        offsetFrequency = getPhotoFrequency();
        /*** EBBY HAS IMPLEMENTED THE FUNCTION ***/
        playCurrentNote();
        /*** FUNCTION TO IMPLEMENT ***/
        averageOffsetFreq = getRunningAverage(getPhotoFrequency());
      }
    }
    /*** STATE B::: *******************************************
...
void playCurrentNote()
{
  ...
  /* ----- EBBY's stuff ----- */
  testNote = pitchIt(analogRead(NOTE_IN_PIN));
  if ( testNote > peakNote) peakNote = testNote;
  if (peakNote < 10) {
    noTone(BUZZER_PIN);
  } else if (peakNote) {
    tone (BUZZER_PIN, (peakNote + offsetFrequency), 100);
  }
...
void updateArraysWithNoteAndTimings()
{
  ...
  notes [countNotes] = peakNote + averageOffsetFreq ;
//  Serial.print("NOTES:    ");
...
int getRunningAverage(int rawSenseVal)
{
  ...
  int avr = currentSum / RUNNING_SAMPLES;
  //  avr = constrain(avr, 0, 255);
  //  Serial.print("AVR :  ");
  //  Serial.println(avr);
  return avr;
  delay(100);
...
/**************************************************************************/
int pitchIt(int inputNote) {
  if (inputNote > 197 && inputNote < 217) return (NOTE_C5);

  else if (inputNote > 247 && inputNote < 267) return (NOTE_D5);

  else if (inputNote > 331 && inputNote < 351) return (NOTE_E5);

  else if (inputNote > 501 && inputNote < 521) return (NOTE_F5);

  else if (inputNote > 1014 && inputNote < 1024) return (NOTE_G5);
